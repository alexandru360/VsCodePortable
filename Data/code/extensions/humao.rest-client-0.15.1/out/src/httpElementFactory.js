    ...
    static getHttpElements(line) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (var variableName in customVariables) {
                originalElements.push(new httpElement_1.HttpElement(variableName, httpElement_1.ElementType.EnvironmentCustomVariable, null, `Value: ${customVariables[variableName]}`, new vscode_1.SnippetString(`{{${variableName}}}`)));
            }
            ...
            for (var [variableName, variableValue] of fileVariables) {
                originalElements.push(new httpElement_1.HttpElement(variableName, httpElement_1.ElementType.FileCustomVariable, '^\\s*[^@]', `Value: ${variableValue}`, new vscode_1.SnippetString(`{{${variableName}}}`)));
            }
            ...
            else if (elements.every(e => e.type === httpElement_1.ElementType.FileCustomVariable)) {
                elements = elements.concat(originalElements.filter(e => !e.prefix));
            }
            else {
                // add global/custom variables anyway
                originalElements.filter(e => !e.prefix && (e.type === httpElement_1.ElementType.SystemVariable || e.type === httpElement_1.ElementType.EnvironmentCustomVariable || e.type === httpElement_1.ElementType.FileCustomVariable)).forEach(element => {
                    elements.push(element);
                });
            }
            return elements;
