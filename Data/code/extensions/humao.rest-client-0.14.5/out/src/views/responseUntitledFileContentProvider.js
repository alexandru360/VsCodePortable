...
class UntitledFileContentProvider {
    static createHttpResponseUntitledFile(response, createNewFile, autoSetLanguage, additionalInfo) {
        ...
        vscode_1.workspace.openTextDocument({ 'language': language, 'content': content }).then(document => {
            vscode_1.window.showTextDocument(document, { viewColumn: vscode_1.ViewColumn.Two, preserveFocus: false, preview: !createNewFile });
        });
    }
    ...
    static formatResponse(response, language, additionalInfo, autoSetLanguage) {
    ...
    static extractStandardResponseInformation(response) {
        ...
        let body = responseFormatUtility_1.ResponseFormatUtility.FormatBody(response.body, response.getResponseHeaderValue("content-type"));
