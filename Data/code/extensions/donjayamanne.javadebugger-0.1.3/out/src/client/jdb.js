...
const waitForPortToOpen_1 = require("./common/waitForPortToOpen");
...
})(JdbCommandType = exports.JdbCommandType || (exports.JdbCommandType = {}));
...
*/
class JdbRunner extends events_1.EventEmitter {
    constructor(args, debugSession) {
        ...
        this.jdbLoaded = new Promise((resolve, reject) => {
            this.jdbLoadedResolve = resolve;
            this.jdbLoadedReject = reject;
        });
        this.readyToAcceptBreakPoints = new Promise((resolve) => {
            this.readyToAcceptBreakPointsResolve = resolve;
        });
        if (contracts_1.isAttachRequestArguments(args)) {
            if (!args.remoteHost) {
    ...
    initProc(port, hostname) {
        ...
        this.jdbProc.stderr.on("data", (data) => {
            ...
            if (this.javaServerAppStarted && this.readyToAcceptCommands) {
                this.debugSession.sendEvent(new vscode_debugadapter_1.OutputEvent(message, "error"));
            }
            else {
                this.exited = true;
        ...
        this.jdbProc.on("error", (data) => {
            if (this.javaServerAppStarted && this.readyToAcceptCommands) {
                ...
                this.debugSession.sendEvent(new vscode_debugadapter_1.OutputEvent("jdb Error " + message, "error"));
            }
            else {
                this.exited = true;
    ...
    startProgramInDebugJavaMode() {
        return getport().then((port) => {
            this.javaLoaded = new Promise((resolve, reject) => {
            ...
            if (options.indexOf("-classpath") !== -1 || options.indexOf("-cp") !== -1) {
                this.debugSession.sendEvent(new vscode_debugadapter_1.OutputEvent("Warning: Specifying -classpath in options of launch.json is deprecated. Please use the classpath option instead.", "console"));
            }
            ...
            if (this.args.externalConsole === true) {
                open_1.open({ wait: false, app: [javaPath].concat(args), cwd: this.args.cwd }).then(proc => {
                    this.javaProc = proc;
    ...
    handleJavaOutput(port) {
        //read the jdb output
        var accumulatedData = "";
        if (this.args.externalConsole) {
            waitForPortToOpen_1.WaitForPortToOpen(port, 5000).then(() => {
                ...
                if (this.javaServerAppStarted && this.readyToAcceptCommands) {
                    this.debugSession.sendEvent(new vscode_debugadapter_1.OutputEvent(message));
                }
                else {
                    this.exited = true;
        ...
        this.javaProc.stdout.on("data", (data) => {
            ...
            if (this.javaServerAppStarted && this.readyToAcceptCommands) {
                if (!this.args.externalConsole) {
                    this.debugSession.sendEvent(new vscode_debugadapter_1.OutputEvent(dataStr, "stdout"));
                }
            }
            else {
        ...
        this.javaProc.stdout.on("close", (data) => {
            if (!this.javaServerAppStarted && !this.readyToAcceptCommands) {
                ...
                this.debugSession.sendEvent(new vscode_debugadapter_1.OutputEvent(accumulatedData));
                return;
            }
            this.onDataReceived("", true);
        });
        this.javaProc.stderr.on("data", (data) => {
            ...
            if (this.javaServerAppStarted && this.readyToAcceptCommands) {
                this.debugSession.sendEvent(new vscode_debugadapter_1.OutputEvent(message, "stderr"));
            }
            else {
                this.exited = true;
    ...
    sendCmd(command, type) {
        ...
        jdbCmd.finalPromise = new Promise(resolve => {
            ...
            promiseToUse.then(() => {
                jdbCmd.promise = new Promise(resolve => {
    ...
    onDataReceived(data, exit = false) {
        ...
        if (this.executingCommands.length === 0 && lastLine.trim().endsWith("]") && this.outputBuffer.indexOf("VM Started") > 0 && !this.vmStarted) {
            ...
            this.vmStarted = true;
            this.outputBuffer = "";
            let startedPromise = Promise.resolve(null);
    ...
    processBreakpoint(lines, lastLine, lastCmd) {
        if (lines.length === 1) {
            return;
        }
    ...
    processCodeStepping(lines, lastLine, lastCmd) {
    ...
    processContinue(lines, lastLine, lastCmd) {
    ...
    processSuspendAndResume(lines, lastLine, lastCmd) {
    ...
    processRunCommand(lines, lastLine, lastCmd) {
    ...
    checkIfBreakpointWillBeHit(lines) {
        return lines.some(line => line.indexOf("Breakpoint hit:") >= 0);
    }
    checkIfDebuggerWillStopDueToInvalidBreakPoints(lines) {
    ...
    checkIfBreakpointWasHitLast(lines, afterCmd) {
        var lastLine = lines[lines.length - 1];
    ...
    checkIfBreakpointWasHit(threadName, data, afterCmd) {
    ...
    breakIfDebuggerStoppedDueToInvalidBreapoints(lines, lastLine, afterCmd) {
        if (this.hasDebuggerStoppedDueToInvalidBreakPoints(lines, lastLine)) {
            this.lastThreadName = lastLine.substring(0, lastLine.indexOf("[")).trim();
            if (afterCmd) {
                //If a command has been passed, then raise the event after that command has been resolved
    ...
    hasDebuggerStoppedDueToInvalidBreakPoints(lines, lastLine) {
    ...
    checkRestOfTheResponse(lines, lastLine, startIndex, indexOfEndOfResponse, lastCmd) {
        if (this.checkIfBreakpointWasHitLast(lines)) {
            return true;
        }
        if (this.breakIfDebuggerStoppedDueToInvalidBreapoints(lines, lastLine, lastCmd)) {
